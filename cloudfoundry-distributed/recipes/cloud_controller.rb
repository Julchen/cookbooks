...
template "#{cloudfoundry_dir}/vcap/cloud_controller/config/cloud_controller.yml" do
  source "cloud_controller.yml.erb"
  owner node[:cloudfoundry][:user][:uid]
  mode 0755
  variables({
    :external_uri => "api.cloudfoundry.rackspace.com",
...
execute "Start cloudfoundry cloud_controller" do
  user node[:cloudfoundry][:user][:uid]
  cwd "#{cloudfoundry_dir}/vcap"
  command "rvm use #{node[:cloudfoundry][:rvm][:default_ruby]} && bin/vcap start cloud_controller"
end
